    ...
    for sub_dir in sub_dirs:  # 遍历目录数组，每次处理一种
        ...
        for extension in extensions:
            file_glob = os.path.join(
                INPUT_DATA, dir_name, '*.' + extension)  # 将多个路径组合后返回
            # glob.glob返回所有匹配的文件路径列表，extend往列表中追加另一个列表
        ...
        for file_name in file_list:  # 遍历此类图片的每张图片的路径
            # 路径的基本名称也就是图片的名称，如：102841525_bd6628ae3c.jpg
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    bottleneck_path = get_bottleneck_path(
        image_lists, label_name, index, category)  # 获取图片特征向量的路径
...
def train():
    ...
    with tf.Session(graph=graph) as sess:
        ...
        constant_graph = graph_util.convert_variables_to_constants(
            sess, sess.graph_def, ["output/prob"])
